...
### QA this round
- `scripts/qa-prod.sh` against the live `api.ayni-ai.com` on GCP with the Mac and the attested Pixel connected: **32/35**. All three misses are `insufficient_credit` on the unfunded QA key (billing enforcement working as designed); `device_attested` quote tier passes. Funding the key through one live checkout turns the suite fully green and exercises the Stripe webhook end to end. The 4 misses are expected for the new stack: 2× `device_attested` (no phone connected to GCP yet) and 2× `insufficient_credit` (the GCP consumer key has a zero balance under `SC_BILLING_ENFORCE=1`; it gets funded through the first live Stripe checkout once the Stripe secrets land).
...
- **Play Console progress (2026-09-06):** app created (`Ayni – Share idle compute`, app id 4974626480826394039, Free, App). Done: privacy policy URL, ads = none, sign-in details (dedicated `play-review-…` provider token, revocable via a new `SC_PROVIDER_TOKENS` version), target audience 18+, data safety (Device IDs + Diagnostics, collected/required/not shared, encrypted in transit, no accounts), government = no, financial features = none, health = none, category Tools, contact e-mail + website, store listing text saved as draft. **Owner clicks left:** upload icon / feature graphic / 2 screenshots ("Add assets" opens a native file picker), IARC content-rating questionnaire (Terms-of-Use checkbox), then Internal testing release with the AAB.
